//! The Boxplot in the `plotters` library only can take the Quartiles from the `plotters` library.
//! Since I am using custom Quartiles, I have to make a custom Boxplot.
...
impl<K: Clone> Boxplot<K, BoxplotOrientV<K, f32>> {
    ...
    ///
    /// ```rust
    /// use plotters::prelude::*; // actually crate::quartiles and such
    ///
    ...
    for &'a Boxplot<K, O>
{
    ...
    fn point_iter(self) -> Self::IntoIter {
        self.values
            .iter()
            .map(|v| O::make_coord(self.key.clone(), *v as f32))
            .collect()
    }
}
